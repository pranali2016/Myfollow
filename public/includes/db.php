...
class DB 
{
	...
	function checkUser($userdata){
		...
		if(mysqli_num_rows($check) > 0){
			...
			$query = "UPDATE $this->userTable SET fname = '".$userdata->firstName."', lname = '".$userdata->lastName."', email = '".$userdata->emailAddress."', location = '".$userdata->location->name."', country = '".$userdata->location->country->code."', profile_url = '".$userdata->publicProfileUrl."', modified = '".date("Y-m-d H:i:s")."' WHERE id = ".$result['id'];
			$this->db->query($query);
			return $result['id'];
		}else{
			$query = "INSERT INTO 
